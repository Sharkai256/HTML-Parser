...
class Element extends Node {
    ...
    🚧 get attributes: AttributeMap // Аттрибуты элемента.
    🚧 get id | set: string // Аттрибут id.
...
class TokenList extends Set<string> {
    🚧 get | set value: string // Классы разделённые пробелом. При присвоении разбивает строку по пробелам.
    🚧 replace: (oldClass: string, newClass: string) => boolean // Заменяет существующий класс новым. Возвращает true если операция успешна.
    🚧 toggle: (class: string, force?: boolean) => boolean // Удаляет класс из списка и возвращает false. Если класса не существует - добавляет и возвращает true.
